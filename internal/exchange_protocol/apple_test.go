...
func TestParseApple(t *testing.T) {
	...
	var merchantID, teamID string
	var privateKeyByte, publicKeyByte, nonceByte []byte

	t.Run("ParseApple", func(t *testing.T) {
		deviceResp, err := ParseApple(sampleHpkeEnvelope, merchantID, teamID, privateKeyByte, publicKeyByte, nonceByte)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if deviceResp.Version != "APPLE_HPKE_V1" {
			t.Fatalf("unexpected error: %v", err)
		}
	})
}
...
func loadPrivateKey() ([]byte, error) {
	...
	// // 型アサーションを使って公開鍵がECDSA形式であることを確認する
	...
	if err != nil {
		return nil, fmt.Errorf("failed to marshal public key to DER format: %v", err)
	}
